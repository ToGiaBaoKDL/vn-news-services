...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch article HTML payloads from Redpanda.")
    parser.add_argument("--group-id", default="article_fetcher")
    ...
    try:
        while not shutdown.requested:
            ...
            if args.once or result:
                return result
        log_event(SERVICE_NAME, "article_fetch_stopped", duration_ms=elapsed_ms(started_at))
        return 0
    finally:
        consumer.close()
...
def process_one(
    ...
    try:
        ...
        event = run_stage(
            "event_decode",
            False,
            lambda: ArticleFetchRequested.model_validate(consumed.decode_value()),
        )
        source = run_stage(
            "source_select",
            False,
